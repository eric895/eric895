...
$(function() {
    ...
    var ctx = '/' + pathName.substring(1, pathName.substring(1).indexOf('/') + 1);
    if (ctx == '/platform') {
        ctx = '';
    }
	/**
			...
			if ( type == 0) {
				...
				ligerComboObj = $(dicCombo).ligerComboBox({
					...
					onSelected:function(newval){
						if(this._toggleSelectBox && (!this.options.isMultiSelect)){
							this._toggleSelectBox(true);
						}
						else if(ligerComboObj && ligerComboObj._toggleSelectBox && (!ligerComboObj.options.isMultiSelect)){
