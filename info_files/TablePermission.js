var TablePermission = {
	init : function(){
		...
		if(typeof permission !="undefined"){
			...
			TablePermission.handlePerTag(permission);//处理表单中的权限控制字段
		}
	},
	/**
	  * 主表的权限补充（必埴）
	...
	/*by byq 分组显示权限控制*/
	handlePerTag : function(permission){
		var teamPermission= permission["teamPermission"];
		if(teamPermission){
			//是否 只读  编辑  隐藏   决定子表的权限
			for(var key in teamPermission){
				var team = teamPermission[key];
				if(!team){
					$("#"+key).hide();
				}
			}
