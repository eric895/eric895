    ...
    CustomForm = {
        ...
        initSubTable: function () {
            $('div[type=subtable]').each(function () {
                ...
                cloneRow.find('[userType="selectorUser"]').show();
                table.data('row', $('<div></div>').append(cloneRow).html());
        ...
        handButton: function (table, canWrite) {
            ...
            if (!canWrite) {
                ...
                $(".oprate-item", table).hide();
                $(".special-td", table).hide();
                return;
            }
            ...
            if (table.menu) {
                ...
                var items = [{
                    ...
                    click: function () {
                        ...
                        // by_byq 个性化修改(删除子表某一行时触发，需要在子表某个字段里增加names="byq_name",并且在统计子表数量的字段里增加id="hjrs")
                        var isdelnum = $("[names='byq_name']").size();
                        if (isdelnum) {
                            console.log(isdelnum);
                            $("#hjrs").val($("[names='byq_name']").size());
                        }
                    }
                }, {
                    text: '向上移动',
        ...
        subSelectorInit: function () {
            ...
            $('[userType="selectorUser"]', '.listTable,[formtype="window"]').each(function () {
                if (!$(this).is(":hidden")) {
                    initSelect2($(this));
                }
            });
