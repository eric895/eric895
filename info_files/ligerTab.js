...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.tab.l = $('<div class="l-tab-l"></div>');
            g.tab.links = $('<div class="l-tab-links"><ul style="left: 0px; "></ul></div>');
            g.tab.links.prependTo(g.tab.l);
            g.tab.l.prependTo(g.tab);
        ...
        setTabButton: function ()
        {
            ...
            var mainwidth = g.tab.links.width();
            if (sumwidth > mainwidth)
            {
            	if($(".l-tab-links-move").length == 0){
            		g.tab.l.append('<div class="l-tab-links-move"><div class="l-tab-links-left"></div><div class="l-tab-links-right"></div></div>');
            		g.setTabButtonEven();
            	}
            	return true;
            } else
            {
                ...
                $(".l-tab-links-move", g.tab.l).remove();
                return false;
            }
        },
        ...
        {
            var g = this, p = this.options;
            $(".l-tab-links-left", g.tab.l).hover(function ()
            {
            ...
            $(".l-tab-links-right", g.tab.l).hover(function ()
            {
        ...
        moveToPrevTabItem: function ()
        {
            ...
            $("li", g.tab.links).each(function (i, item)
            {
                ...
                if (i > 0)
                {
                    currentItemLeft = parseInt(leftList[i - 1]) + $(this).prev().outerWidth(true);
                }
        ...
        moveToNextTabItem: function ()
        {
            ...
            for (var i = tabItems.length - 1; i >= 0; i--)
            {
                ...
                {
                    currentItemLeft = parseInt(leftList[tabItems.length - 2 - i]) - $(tabItems[i + 1]).width();
                }
                leftList.push(currentItemLeft);
            }
            var currentLeft = -1 * parseInt(g.tab.links.ul.css("left"));
...
//            $("> .l-tab-content-item[tabid=" + tabid + "]", g.tab.content).show().siblings().hide();
        ...
        moveToLastTabItem: function ()
        {
            ...
            {
                //var btnWitdth = $(".l-tab-links-right", g.tab.links).width();
                g.tab.links.ul.animate({ left: -1 * (sumwidth - mainwidth) });
            }
        },
        //判断tab是否存在
        ...
        removeTabItem: function (tabid)
        {
            ...
            var liWidth = $("li[tabid=" + tabid + "]", g.tab.links.ul).outerWidth(true);
            $("li[tabid=" + tabid + "]", g.tab.links.ul).remove();
            var flag =g.setTabButton();
            ...
            var leftCss = parseInt(g.tab.links.ul.css("left").split("p")[0]);
            if( leftCss + liWidth <= 0 && flag){
            	g.tab.links.ul.animate({ left: leftCss+liWidth });
            }
        },
