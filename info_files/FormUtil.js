...
FormUtil.initCalendar=function(){
    ...
    if(isMobile()){
        $('input.Wdate').each(function() {
            ...
            var showdatefmt = $(this).attr('datefmt');
            $(this).attr('readonly', true);
            if (datefmt === 'yyyy-MM-dd HH:mm:00') {
                datefmt = 'yyyy-MM-dd HH:mm';
                showdatefmt = datefmt.replace('HH', 'hh');
            } else if (datefmt.indexOf('HH') != -1) {
                showdatefmt = datefmt.replace('HH', 'hh');
            }
            //默认显示
            if ($(this).attr('displaydate') === '1') {
                if ($(this).val() === '') {
                    $(this).val(new Date().format(showdatefmt));
                }
            }

            var type = '';
            //laydate类型
            if (datefmt.indexOf('yyyy') !== -1 && datefmt.indexOf('HH') !== -1) {
                type = 'datetime';
            } else if (datefmt.indexOf('MM') !== -1 && datefmt.indexOf('dd') == -1 ) {
                type = 'month';
            ...
            laydate.render({
                ...
                format: datefmt,
                    ...
                    // update by byq 强加日期计算触发方式，这个方法在关闭时触发
                    $("input[datecalculate]").each(function(){
                        ...
                        try{
                            ...
                            if(name.indexOf(dateCalculateStart) >= 0){
                                startTime = value;
                            }
                            if(name.indexOf(dateCalculateEnd) >= 0){
                                endTime = value;
                            }
                            var value_ = FormDate.getDateDiffVal(startTime, endTime, diffType);
                            me.val(value_);
                            me.trigger("change");
                            me.blur();
                        }catch(e){
	...
	for(var i=0,c;c=choice[i++];){
		...
		if(changes){
			for(var i=0;i<changes.length;i++){
				var type = $("[name='"+changes[i].key+"']").attr("type");
				var ctltype = $("[name='"+changes[i].key+"']").attr("ctltype");
				var controltype = $("[name='"+changes[i].key+"']").attr("controltype");
				var validRule = $("[name='"+changes[i].key+"']").attr("validate");
				var obj = $("[name='"+changes[i].key+"']");
				if(typeof ctltype !== typeof undefined && ctltype !== false) {
					if(obj.parent().parent().is('div')) {
						if (c.type == "1") {
					...
					if(obj.parent().parent().is('td')){
						if(c.type=="1"){
							if(c.key.indexOf("s:")==-1||c.key.indexOf("s:")>2){
				...
				}else if(obj.parent().is('td')){
					if(changes[i].type=="1"){
						if(changes[i].key.indexOf("s:")==-1||changes[i].key.indexOf("s:")>2){
				...
				}else if(obj.parent().is('span')&&obj.parent().hasClass("select_contain_span")){
				//必填时会增加span元素
					if(obj.parent().parent().is('div')){
						if(changes[i].type=="1"){
					...
					}else if(obj.parent().parent().is('td')){
						if(changes[i].type=="1"){
							if(changes[i].key.indexOf("s:")==-1){
								obj.parent().parent().prev().hide();
							}
							obj.parent().parent().hide();
						}else if(changes[i].type=="2"){
                ...
                if(changes[i].type=="3"){
                    ...
                    if(validRule != null && validRule != ""){
                        ...
                        if(!json.required){
                            ...
                            obj.attr("validate",jsonStr);
                            if(json.required == false){
                                var jsonStr2 = validRule.substring(0, validRule.lastIndexOf(','));
                                //加上必填
                                jsonStr2 +=",\'required\':true}";
                                obj.attr("validate",jsonStr2);
                            }
                        }
                        obj.addClass("red");
                    }
...
FormUtil.doChangeField = function(choiceKey,c,$tab){
    var type = $("[name='"+c.key+"']").attr("type");
    var ctltype = $("[name='"+c.key+"']").attr("ctltype");
    var controltype = $("[name='"+c.key+"']").attr("controltype");
    var validRule = $("[name='"+c.key+"']").attr("validate");
    var obj = $("[name='"+c.key+"']");
    if(typeof ctltype !== typeof undefined && ctltype !== false) {
		if(obj.parent().parent().is('div')) {
			if (c.type == "1") {
	 ...
	 for(var i=0,c;c=d[i++];){
		 ...
		 for(var j=0,m;m=choisefield[j++];){
			 ...
			 changeObj.die("change").live("change",function(e){
				 $changeFunc = [];
				 $changeObj = $(e.target);
				 console.log("changeObj1-----"+$changeObj);
				 var key = $changeObj.attr("name");
				 console.log("key----"+key);
				 var type = $changeObj.attr("type");
				 ...
				 for (var x = 0, n; n = d[x++];) {
					 if (n.choisefield.indexOf(key) > -1) {
						 ...
						 for (var i = 0; i < curChoise.length; i++) {
							 var value = curChoise[i].value;
							 var nowvalue = $('[name="' + key + '"]').val();
							 if ('radio' == type) {
								 nowvalue = $('[name="' + key + '"]:checked').val();
							 }
							 if (value == nowvalue) {
			 ...
			 console.log("changeObj2----"+changeObj);
			 changeObj.trigger("change").trigger("blur");
		 }
	 }
